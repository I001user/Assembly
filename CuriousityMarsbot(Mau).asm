...
ReadKey:	lw $t6, 0($k0)			#$t6 = [$k0] = KEY_CODE
		...
		bne $t6, '\n' , loop
CheckControlCode:
		la $s2, lengthControlCode	#Gia tri (content) cua $s2: dia chi cua lengthControlCode
		lw $s2, 0($s2)			#load gia tri  (content) cua 0($s2)-tuong-duong-> 0(gia tri cua $s2)-tuong-duong->0(lengthControCode) vao trong $s2
		#----------------
...
#				inputControlCode = "", makes all char of inputControlCode become '\0'
# param[in] none
#-----------------------------------------------------------				
removeControlCode:
	#backup
...
scan_row1:					
	li $t3, 0x81			#scan_row1 and enable bit 7 (hence 0x81 not 0x01, bit 7 must be enabled for interrupt to happen
	sb $t3, 0($t1)
	lbu $a0, 0($t2)
	bnez $a0, get_code_in_char
scan_row2:					
	li $t3, 0x82			#scan_row2 and enable bit 7
	sb $t3, 0($t1)
	lbu $a0, 0($t2)
	bnez $a0, get_code_in_char
scan_row3:					
	li $t3, 0x84			#scan_row3 and enable bit 7
	sb $t3, 0($t1)
	lbu $a0, 0($t2)
	bnez $a0, get_code_in_char
scan_row4:					
	li $t3, 0x88			#scan_row4 and enable bit 7
	sb $t3, 0($t1)
	lbu $a0, 0($t2)
	bnez $a0, get_code_in_char
get_code_in_char:				
	beq $a0, KEY_0, case_0		#get_code_in_char
	beq $a0, KEY_1, case_1
	beq $a0, KEY_2, case_2
	beq $a0, KEY_3, case_3
	beq $a0, KEY_4, case_4
...
case_f:	li $s0, 'f'
	j store_code
store_code:					
	la $s1, inputControlCode		#store_code
	la $s2, lengthControlCode
	lw $s3, 0($s2)				#$s3 = strlen(inputControlCode)
	addi $t4, $t4, -1 			#$t4 = i 
	for_loop_to_store_code:			
		addi $t4, $t4, 1		#for_loop_to_store_code
